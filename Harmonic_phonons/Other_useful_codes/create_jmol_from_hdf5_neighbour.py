...
def plot_molecule_com(atoms):
    molecule_indices, molecule_coms = identify_molecules(atoms)
    
    # Convert molecule COMs to numpy array for easier handling
    molecule_coms = np.array(molecule_coms)
    
    # Create a scatter plot
    fig = go.Figure()
    
    # Add molecule COM points
    fig.add_trace(go.Scatter3d(
        ...
        text=[str(i) for i in range(len(molecule_coms))],  # Label each COM with its molecule index
        textposition='top center'
    ))
    
    # Layout settings
    fig.update_layout(
        title='3D Plot of Molecule Centers of Mass',
        scene=dict(
            xaxis_title='X (Å)',
            yaxis_title='Y (Å)',
            zaxis_title='Z (Å)'
...
# # mol_idx_to_keep = [111, 112, 25, 8, 41, 192, 159, 126, 176, 143, 206]


# pentacene_idx = np.array([idx for idx in mol_idx[nearest_neighbors[0]]])
# # print(f'Pentacene indices: {pentacene_idx}')

# guest_nn_idx = np.array([idx for neighbor in nearest_neighbors[1:] for idx in mol_idx[neighbor]])
# # print(f'Guest nearest neighbor indices: {guest_nn_idx}')

# atom_indices_of_nn_atoms = np.array([idx for neighbor in nearest_neighbors for idx in mol_idx[neighbor]])


# # Reading the hdf5 file
# mode_index = 0
# scaling_factor_pentacene = 100
# scaling_factor_nn_atoms = 10
...
# pentacene_positions = positions[pentacene_idx]
# pentacene_displacements = displacements[pentacene_idx]
# pentacene_atom_types = np.array(atom_types)[pentacene_idx]

# # Extracting the guest nearest neighbor mode
# nn_positions = positions[guest_nn_idx]
# nn_displacements = displacements[guest_nn_idx]
# nn_atom_types = np.array(atom_types)[guest_nn_idx]
...
# if not os.path.exists(f'mode_index_1'):
#     os.mkdir(f'mode_index_1')
# os.chdir(f'mode_index_1')

# with open(f"Host-guest_mode_1.xyz", "w") as f:
