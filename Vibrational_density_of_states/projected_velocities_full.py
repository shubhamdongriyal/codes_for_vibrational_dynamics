...
masses = np.diag(np.sqrt(velTraj[0].get_masses()))
projections_sum = []
for i in range(1, len(velTraj)):
    projections = np.real(eigenvectors_reshaped[modeIndex] * (masses @ velTraj[i].get_positions()))
    projection_sum = np.sum(projections, axis=(0, 1))
    projections_sum.append(projection_sum)
    # print(f'Projections sum: {projections_sum}')

np.savetxt(f'full_projected_velocities_{modeIndex}_{filename}.txt', projections_sum)
