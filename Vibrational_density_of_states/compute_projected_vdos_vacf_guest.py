...
def read_trajectories(nruns, mdlen):
    ...
    for i in range(1, nruns + 1):
        if i + 4 < 10:
            filename = f'guest_projected_velocities_{modeIndex}_guest100K-nve.velocities_0_0{i+4}.xyz.txt'
            data = np.loadtxt(filename)
            projModes1.append(data) 
        else:
            filename = f'guest_projected_velocities_{modeIndex}_guest100K-nve.velocities_0_{i+4}.xyz.txt'
            data = np.loadtxt(filename)
            projModes1.append(data)
    return projModes1
