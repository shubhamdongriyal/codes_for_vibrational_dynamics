...
def read_trajectories(nruns, mdlen):
    ...
    for i in range(1, nruns+1):
        if i+4 < 10:
            filename = f'/ada/ptmp/mpsd/shubsharma/HDNNP/mace/naphthalene_com_tight/host_guest/analysis/eigen_mode_projection/new_compute/mode-3/full/velocities/full_projected_velocities_3_nap100K-nve.velocities_0_0{i+4}.xyz.txt'

        else:
            filename = f'/ada/ptmp/mpsd/shubsharma/HDNNP/mace/naphthalene_com_tight/host_guest/analysis/eigen_mode_projection/new_compute/mode-3/full/velocities/full_projected_velocities_3_nap100K-nve.velocities_0_{i+4}.xyz.txt'

        data = np.loadtxt(filename)
        projModes1.append(data)
        
    
    # files = sorted(glob.glob('full_projected_velocities_*_nap100K-nve.velocities_0_*.xyz.txt'))
    # files_to_process = files[:nruns]

    # for filename in files_to_process:
    #     # Load data from each file
    #     data = np.loadtxt(filename)
    #     projModes1.append(data/0.5291772105638411)
...
print(len(projModes1))

if len(projModes1) != 27:
    print(f'Number of trajectories read: {len(projModes1)}')
    print('Exiting...')
    sys.exit(1)
...
for j in range(len(projModes1)):  
    vel = np.asanyarray([projModes1[j][i] * (1/0.5291772105638411 if j > 10 else 1) for i in range(len(projModes1[j]))])
