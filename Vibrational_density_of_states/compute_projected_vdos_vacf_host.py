...
def read_trajectories(nruns, mdlen):
    ...
    for i in range(1, nruns + 1):
        try:
            ...
            if i + 4 < 10:
                filename = f'host_projected_velocities_{modeIndex}_host100K-nve.velocities_0_0{i+4}.xyz.txt'
            else:
                filename = f'host_projected_velocities_{modeIndex}_host100K-nve.velocities_0_{i+4}.xyz.txt'
...
print(f'\n')
# os.mkdir(f'guest_projected_{num_traj}_traj_{mdlen}_ps')
# os.chdir(f'guest_projected_{num_traj}_traj_{mdlen}_ps')
...
np.savetxt(f'lifetimes_info_{num_traj}_traj_{mdlen}_ps.txt', np.hstack((lifetime_mean, lifetime_std, gamma_mean, gamma_std)), delimiter='\t', header='Lifetime_mean (s)\tLifetime_std (s)\tGamma_mean (cm-1)\tGamma_std (cm-1)')
