...
masses = np.diag(np.concatenate((np.sqrt(velTraj[0].get_masses()), np.zeros((36)))))
projections_sum = []
for i in range(1, len(velTraj)):
    velTraj[i]=np.vstack((velTraj[i].get_positions(), np.zeros((36, 3))))
...
np.savetxt(f'host_projected_velocities_{modeIndex}_{filename}.txt', projections_sum)
